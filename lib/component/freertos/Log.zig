...
pub fn write(
    ...
    const line = embed.fmt.bufPrintZ(
        &line_buf,
        "[{s}] [{s}] " ++ format ++ "\n",
        .{ level_text, scope_text } ++ args,
    ) catch {
        const fallback = embed.fmt.bufPrintZ(
            &line_buf,
...
fn levelText(comptime level: anytype) []const u8 {
    ...
    if (comptime embed.mem.eql(u8, name, "err")) return "error";
    if (comptime embed.mem.eql(u8, name, "warn")) return "warning";
    return name;
}
