...
pub fn expectEqualStrings(expected: []const u8, actual: []const u8) !void {
    if (!embed.mem.eql(u8, expected, actual)) {
        return error.TestExpectedEqual;
    }
}
...
fn testValuesEqual(expected: anytype, actual: anytype) bool {
    ...
    return embed.meta.eql(expected, actual);
}

fn compareComptimeIntToValue(comptime expected: comptime_int, actual: anytype) bool {
    return switch (@typeInfo(@TypeOf(actual))) {
        .int, .comptime_int => embed.meta.eql(@as(@TypeOf(actual), @intCast(expected)), actual),
        .float, .comptime_float => embed.meta.eql(@as(@TypeOf(actual), @floatFromInt(expected)), actual),
        else => false,
    };
}
...
fn compareComptimeFloatToValue(comptime expected: comptime_float, actual: anytype) bool {
    return switch (@typeInfo(@TypeOf(actual))) {
        .float, .comptime_float => embed.meta.eql(@as(@TypeOf(actual), @floatCast(expected)), actual),
        else => false,
    };
}
