...
pub fn main() !void {
    ...
    if (args.len < 6 or args.len > 7) {
        std.debug.print(
            "usage: data_partitions <build|flash> <app_root> <build_dir> <esp_idf> <wrapper_path> [port]\n",
            .{},
        );
    ...
    if (std.mem.eql(u8, mode, "build")) {
        try buildDataPartitionImages(allocator, app_root, build_dir, idf_root, wrapper_path);
        return;
    }
    if (std.mem.eql(u8, mode, "flash")) {
        try flashBuiltDataPartitionImages(allocator, app_root, build_dir, idf_root, wrapper_path, port);
        return;
    }
    return error.InvalidArgs;
}
...
pub fn buildDataPartitionImages(
    ...
    for (images) |image| {
        try buildDataPartitionImage(allocator, app_root, build_dir, idf_root, wrapper_path, image);
    }
}
...
pub fn flashBuiltDataPartitionImages(
    ...
    for (images) |image| {
        try flashBuiltDataPartitionImage(allocator, idf_root, wrapper_path, port, image);
    }
}
