...
fn parseCodeSymbols(
    ...
    var out = try allocator.alloc(CodeSymbol, count);
    var used: usize = 0;
...
fn summarizeModules(allocator: std.mem.Allocator, code_symbols: []const CodeSymbol) ![]ModuleStat {
...
pub fn main() !void {
    ...
    const module_stats = try summarizeModules(allocator, code_symbols);
    defer allocator.free(module_stats);
