...
pub fn main() !void {
    ...
    if (args.len != 6) {
        std.debug.print(
            "usage: combine_flash_image <app_root> <build_dir> <esp_idf> <wrapper_path> <output>\n",
            .{},
        );
    ...
    const esp_idf = args[3];
    const wrapper_path = args[4];
    ...
    if (esp_idf.len != 0) {
        ...
        try argv.appendSlice(&.{ "python3", "-m", "esptool" });
    }

    try argv.appendSlice(&.{
        "--chip",
        build_config.board.chip,
        "merge_bin",
        "-o",
        output_path,
        "-f",
