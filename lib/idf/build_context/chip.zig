...
pub fn resolveToolchainSysroot(
    ...
    const root = runCompilerQuery(b, compiler, "-print-sysroot", esp_idf, wrapper_path) orelse return null;
    return .{
        .root = root,
...
fn runCompilerQuery(
    ...
    if (esp_idf) |idf_root| {
        argv.appendSlice(b.allocator, &.{ "bash", wrapper_path, idf_root, compiler, arg }) catch return null;
    } else {
        argv.appendSlice(b.allocator, &.{ compiler, arg }) catch return null;
    }
