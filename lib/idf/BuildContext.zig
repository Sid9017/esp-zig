...
/// Binding module used by C-facing glue code generated for the app build.
...
/// IDF component names derived from `build_config.config` for this app.
configured_idf_component_names: []const []const u8,
...
pub fn resolve(
    ...
    const binding_module = esp_dep.module("esp_binding");
    binding_module.addImport("build_config", opts.build_config);
    binding_module.addImport("esp_idf", esp_dep.module("esp_idf"));
    ...
    const idf_py_executable_path = b.pathJoin(&.{ idf_path, "tools", "idf.py" });
    return .{
        ...
        .configured_idf_component_names = processed_build_config.configured_idf_component_names,
        ...
        .toolchain_sysroot = chip_mod.resolveToolchainSysroot(
            b,
            processed_build_config.chip,
            maybe_idf_path,
            esp_root.path(b, "lib/idf/tools/idf_env_wrapper.sh").getPath(b),
        ),
    };
}
...
pub fn extract(context: Self, b: *std.Build) Extracted {
    return .{
        ...
        .configured_idf_component_names = b.dupeStrings(context.configured_idf_component_names),
...
fn processBuildConfig(
    ...
    const probe_source = std.fmt.allocPrint(
        ...
        \\    try stdout.writeAll(build_config.board.chip);
        \\    try stdout.writeAll("\n");
        \\    const names = esp_idf.SdkConfig.coreOverrideNames(build_config.config);
        \\    for (names) |name| {{
        \\        try stdout.writeAll("component=");
    ...
    while (iter.next()) |line| {
        ...
        if (std.mem.startsWith(u8, trimmed, "chip=")) {
            ...
            if (value.len == 0) {
                std.debug.panic("build_config board.chip is empty in '{s}'", .{build_config_path});
            }
            chip = b.allocator.dupe(u8, value) catch @panic("OOM");
            continue;
        }
        if (std.mem.startsWith(u8, trimmed, "component=")) {
            const value = trimmed["component=".len..];
            if (value.len == 0) continue;
            names.append(b.allocator, b.dupe(value)) catch @panic("OOM");
            continue;
        }
        std.debug.panic(
    ...
    return .{
        .chip = chip orelse std.debug.panic(
            "missing board.chip in build_config probe for '{s}'",
            .{build_config_path},
        ),
        .configured_idf_component_names = names.toOwnedSlice(b.allocator) catch @panic("OOM"),
    };
}
