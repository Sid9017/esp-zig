...
pub fn addSdkconfigGeneratorTool(
    ...
    const board_adapter_module = b.createModule(.{
        .root_source_file = context.esp_zig_root.path(b, "lib/idf/sdk_config/board_adapter.zig"),
        .imports = &.{
            .{ .name = "esp_idf", .module = context.esp_idf_module },
            .{ .name = "app_sdkconfig_profile", .module = context.build_config_module },
        },
    });
    const run = addBuiltinHostTool(
        ...
        &.{
            .{ .name = "esp_idf", .module = context.esp_idf_module },
            .{ .name = "app_sdkconfig_profile", .module = context.build_config_module },
            .{ .name = "board_adapter", .module = board_adapter_module },
        },
    );
...
pub fn addGenerateAddappProjectTool(
    ...
    for (project.requirements) |requirement| {
        ...
        run.addArg(b.fmt("{d}", .{component.files.len + component.archives.len}));
        ...
        for (component.files) |file| {
            run.addArg(file.idf_project_path);
            run.addFileArg(file.original_path);
        }
...
pub fn addDataPartitionBuildTool(
    ...
    run.addFileArg(context.esp_zig_root.path(b, "lib/idf/tools/idf_env_wrapper.sh"));
    return &run.step;
}
...
pub fn addFlashCombinedImageTool(
    ...
    const run = blk: {
        const wrapped = b.addSystemCommand(&.{"bash"});
        wrapped.addFileArg(context.esp_zig_root.path(b, "lib/idf/tools/idf_env_wrapper.sh"));
        wrapped.addArg(context.idf_path);
        wrapped.addArg("python3");
        break :blk wrapped;
    };
...
pub fn addRestoreMonitorElfTool(
    b: *std.Build,
    context: BuildContext.BuildContext,
    app_name: []const u8,
) *std.Build.Step {
    const run = addBuiltinHostTool(
        b,
        "restore_monitor_elf",
        context.esp_zig_root.path(b, "lib/idf/tools/restore_monitor_elf.zig"),
        &.{},
    );
    run.setCwd(b.path(context.app_root));
    run.addArg(b.pathJoin(&.{ context.binary_output_dir, b.fmt("{s}.elf", .{app_name}) }));
    run.addArg(b.pathJoin(&.{ context.build_dir, b.fmt("{s}.elf", .{app_name}) }));
    return &run.step;
}
