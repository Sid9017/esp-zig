...
fn extractWithPrefix(component: *const Self, idf_path_prefix: []const u8) !Extracted {
    ...
    const files = try b.allocator.alloc(Extracted.File, sourceFileCount(component) + objectArtifactCount(component));
...
pub fn extractTo(component: *const Self, dir_path: []const u8) !void {
    ...
    for (extracted.files) |file| {
        const destination_path = try std.fs.path.join(b.allocator, &.{ dir_path, file.idf_project_path });
        defer b.allocator.free(destination_path);
        try fs_utils.copyFileAbsoluteOrRelative(file.original_path.getPath(b), destination_path);
    }
...
pub fn validate(component: *const Self) void {
    if (!component.hasContent()) {
        std.debug.panic(
            "idf.Component '{s}' must declare at least one source file or artifact",
            .{component.name},
        );
    }
}
...
fn writeComponentCmakeLists(extracted: Extracted, target_dir: []const u8) !void {
    ...
    const needs_stub = !hasCmakeSourceFiles(extracted.files) and
        (extracted.archives.len != 0 or hasObjectFiles(extracted.files));
    if (needs_stub) {
    ...
    if (needs_stub) {
        ...
        for (extracted.files) |file| {
            if (isObjectFile(file.idf_project_path) or isArchiveFile(file.idf_project_path)) continue;
            try writer.print("        \"{s}\"\n", .{file.idf_project_path});
        }
    }
    try writer.writeAll(
        "    INCLUDE_DIRS\n" ++
    ...
    for (extracted.files) |file| {
        if (!isObjectFile(file.idf_project_path)) continue;
        try writer.print(
...
fn hasCmakeSourceFiles(files: []const Extracted.File) bool {
    for (files) |file| {
        if (!isArchiveFile(file.idf_project_path) and !isObjectFile(file.idf_project_path)) return true;
    }
    return false;
}

fn hasObjectFiles(files: []const Extracted.File) bool {
    for (files) |file| {
        if (isObjectFile(file.idf_project_path)) return true;
    }
    return false;
}
...
test "extract returns lean component description" {
    ...
    try std.testing.expectEqual(@as(usize, 1), extracted.files.len);
    ...
    try std.testing.expectEqualStrings("components/esp_main_helper/wifi.c", extracted.files[0].idf_project_path);
    try std.testing.expectEqualStrings(expected_source, extracted.files[0].original_path.getPath(b));
...
test "writeComponentCmakeLists writes external object artifacts into component CMakeLists" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, staged_cmake, "idf_shim") != null);
}
...
test "extract keeps multiple source roots under their shared ancestor" {
    ...
    try tmp.dir.makePath("lib/component/esp_hw_support");
    try tmp.dir.makePath("lib/component/freertos");
    try tmp.dir.writeFile(.{
        .sub_path = "lib/component/esp_hw_support/binding.c",
        .data = "void esp_hw_support_binding(void) {}\n",
    });
    try tmp.dir.writeFile(.{
        .sub_path = "lib/component/freertos/binding.c",
        .data = "void freertos_binding(void) {}\n",
    });

    const tmp_root = try std.fmt.allocPrint(arena, ".zig-cache/tmp/{s}", .{tmp.sub_path});
    const esp_hw_support_root = try std.fs.path.join(arena, &.{ tmp_root, "lib", "component", "esp_hw_support" });
    const freertos_root = try std.fs.path.join(arena, &.{ tmp_root, "lib", "component", "freertos" });

    const component = create(b, .{ .name = "idf_shim" });
    component.addCSourceFiles(.{
        .root = .{ .cwd_relative = esp_hw_support_root },
        .files = &.{"binding.c"},
    });
    component.addCSourceFiles(.{
        .root = .{ .cwd_relative = freertos_root },
        .files = &.{"binding.c"},
    });
    component.addIncludePath(.{ .cwd_relative = esp_hw_support_root });
    component.addIncludePath(.{ .cwd_relative = freertos_root });

    const extracted = try component.extract("components/idf_shim");

    try std.testing.expectEqual(@as(usize, 2), extracted.files.len);
    try std.testing.expectEqual(@as(usize, 2), extracted.include_dirs.len);
    try std.testing.expectEqualStrings(
        "components/idf_shim/esp_hw_support/binding.c",
        extracted.files[0].idf_project_path,
    );
    try std.testing.expectEqualStrings(
        "components/idf_shim/freertos/binding.c",
        extracted.files[1].idf_project_path,
    );
    try std.testing.expectEqualStrings(
        "components/idf_shim/esp_hw_support",
        extracted.include_dirs[0].idf_project_path,
    );
    try std.testing.expectEqualStrings(
        "components/idf_shim/freertos",
