...
pub fn create(
    ...
    _ = project.addIdfShimComponent();
    _ = project.addIdfComponents();
...
fn addIdfShimComponent(project: *Self) *Component {
    const bctx = project.context orelse
        std.debug.panic("idf.Project.addIdfShimComponent() requires project.context", .{});
    ...
    for (SdkConfig.required_field_names) |component_name| {
        component.addRequire(component_name);
        const source_dir = bctx.esp_zig_root.path(project.owner, project.owner.fmt("lib/component/{s}", .{component_name}));
        var dir = fs_utils.openDirAbsoluteOrRelative(source_dir.getPath(project.owner), .{ .iterate = true }) catch |err| {
            if (err == error.FileNotFound) continue;
            std.debug.panic(
                "failed to open idf shim source dir for '{s}': {s}",
                .{ component_name, @errorName(err) },
            );
        };
        defer dir.close();
        ...
        while (iter.next() catch |err| std.debug.panic(
            "failed to iterate idf shim dir for '{s}': {s}",
            .{ component_name, @errorName(err) },
        )) |entry| {
            if (entry.kind != .file) continue;
            if (!std.mem.endsWith(u8, entry.name, ".c")) continue;
            source_files.append(project.owner.allocator, project.owner.dupe(entry.name)) catch @panic("OOM");
        }

        if (source_files.items.len == 0) continue;
        ...
        component.addIncludePath(source_dir);
    }

    if (!has_sources) {
        const stub_files = project.owner.addWriteFiles();
        const stub_source = stub_files.add("idf_shim_stub.c", "void espz_idf_shim_stub(void) {}\n");
...
fn addIdfComponents(project: *Self) []*Component {
    const bctx = project.context orelse
        std.debug.panic("idf.Project.addIdfComponents() requires project.context", .{});
    for (bctx.configured_idf_component_names) |component_name| {
        addRequirement(project, component_name, null);
        project.addEntryRequire(component_name);
    }
    return project.owner.allocator.alloc(*Component, 0) catch @panic("OOM");
}
...
test "extract returns lean idf project description" {
    ...
    try std.testing.expectEqualStrings("components/esp_main_helper/wifi.c", extracted.requirements[2].component.?.files[0].idf_project_path);
