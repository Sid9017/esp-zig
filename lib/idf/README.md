...
5. Resolves the ESP toolchain sysroot for C archive compilation.
6. Captures the caller's `app_root` and `build_dir` so later staging steps do not have to re-derive them.
...
| `idf_module` | The public Zig `idf` module | Imported by build-time host tools and app-side modules |
| `esp_root` | The root of the `esp-zig` package | Used to locate helper tools like `generate_app_main.zig`, `generator.zig`, `idf_env_wrapper.sh`, etc. |
| `esp_idf` | The resolved ESP-IDF root path, or `null` | Passed into `idf.py`, environment wrapper scripts, and sysroot resolution |
...
| `monitor` | Restores the monitor ELF path and runs `idf.py monitor` without flashing first |
| `flash_monitor` | Flashes the combined firmware image, then runs `idf.py monitor` |
...
flowchart LR
    ...
    configure --> restoreElf[restore_monitor_elf]
    restoreElf --> monitor[monitor]
    combine --> flash[flash]
    flash --> flashMonitor[flash_monitor]
    restoreElf --> flashMonitor
```
...
- `monitor` depends on the monitor-ELF restore step
- `flash_monitor` depends on both `flash` and the monitor-ELF restore step
